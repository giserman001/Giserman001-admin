...
export default class ColorPalette {
  constructor() {
    ...
    const table = h('table', '').children(
      h('tbody', '').children(
        h('tr', `${cssPrefix}-theme-color-placeholders`).children(...themeColorPlaceHolders.map((color) => buildTd.call(this, color))),
        ...themeColors.map((it) => h('tr', `${cssPrefix}-theme-colors`).children(...it.map((color) => buildTd.call(this, color)))),
