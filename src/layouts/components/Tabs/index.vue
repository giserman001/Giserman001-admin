<template>
	<div class="ly-tabs">
		<a-tabs v-model:activeKey="tabsMenuValue" hide-add type="editable-card"
			:tabBarStyle="{ margin: '4px 0 12px', padding: '0 10px', gap: '6px' }" class="ly-bg-[#fff]" @tabClick="tabClick" @edit="onEdit">
			<template #leftExtra>
			...
			<a-tab-pane v-for="item in tabsMenuList" :key="item.path" :closable="item.close">
				<template #tab>
					<span ly-flex ly-items-center ly-gap="4px">
						<SvgIcon :name="item.icon" width="16px" height="16px" ly-mr="0!" v-if="item.icon && tabsIcon" />
						{{ item.title }}
					</span>
				</template>
			</a-tab-pane>
...
import { LeftOutlined, RightOutlined, MoreOutlined } from '@ant-design/icons-vue'
