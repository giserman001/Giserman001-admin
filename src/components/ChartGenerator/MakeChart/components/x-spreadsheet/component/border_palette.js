    ...
    const table = buildTable(
      h('tr', '').children(
        h('td', `${cssPrefix}-border-palette-left`).child(
          buildTable(
            h('tr', '').children(...['all', 'inside', 'horizontal', 'vertical', 'outside'].map((it) => buildTd.call(this, it))),
            h('tr', '').children(...['left', 'top', 'right', 'bottom', 'none'].map((it) => buildTd.call(this, it)))
          )
        ),
        h('td', `${cssPrefix}-border-palette-right`).children(h('div', `${cssPrefix}-toolbar-btn`).child(this.ddColor.el), h('div', `${cssPrefix}-toolbar-btn`).child(this.ddType.el))
      )
    )
    this.el.child(table)
  }
