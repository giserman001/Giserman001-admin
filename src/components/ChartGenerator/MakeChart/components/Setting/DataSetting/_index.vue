  ...
  <div class="DataSetting">
    <a-form v-if="data">
      ...
      <a-form-item v-if="dataOrgion === 'api'" :label="$t('接口地址')" name="api" :rules="null">
        <a-input-search v-model:value="total.inerfaceurl" :enter-button="$t('测试')" @search="onTest(total.inerfaceurl)"> </a-input-search>
      </a-form-item>
      <LineOrBar v-if="[chartType.line, chartType.bar].includes(total.chartType)" :data="data" :total="total" :dataOrgion="dataOrgion" />
