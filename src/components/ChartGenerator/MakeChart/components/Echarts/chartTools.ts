...
// 利润增加/减少的提示
export const diffTip = ({ marker, seriesName, value }, { index, data, diffValOrPer, yuceKey = '预测净利润', shijiKey = '实际净利润', hiddenIndex = null }) => {
  ...
  if (shiji) {
    ...
    if (diffValOrPer === '1') {
      ...
      // 百分比
      dopTip = ` <span style='display:inline-block;width:15px'></span>利润 <span style="color: ${colors[difference >= 0 ? 'up' : 'down']}">${difference >= 0 ? '增加' : '减少'}: ${diff}</span>`
    }
  }
